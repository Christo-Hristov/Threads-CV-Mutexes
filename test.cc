...
block()
{
    ...
    // be woken up again.
    Thread *thread1 = new Thread(block_thread1);
    new Thread([thread1] {block_thread2(thread1);});
    intr_enable(false);
    Thread::redispatch();
}
...
exit2()
{
    ...
    // This test is no longer part of the test suite, because it would
    // require the use of deque for the ready queue instead of queue.
...
stack_thread(int thread_num)
{
    std::cout << "Thread " << thread_num << " woke up (" << Stack::num_stacks_
            << " stacks currently exist); exiting" << std::endl;
    Thread::exit();
}
...
mine_thread1(Mutex &m)
{
    ...
    m.lock();
    std::cout << "thread 1 reacquired    lock" << std::endl;
    std::cout << "thread 1 calling mine() after reacquiring lock: returns "
              << m.mine() << std::endl;
}
...
main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // Tests below here are for the mutex/condition variable assignment
        } else if (strcmp(argv[i], "mutex_basic") == 0) {
            mutex_basic();
        } else if (strcmp(argv[i], "many_threads") == 0) {
            many_threads();
